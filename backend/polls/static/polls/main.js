            ...
            let find_panorama = function() {
                streetview.getPanorama(panorama_request, function(response, status) {
                    if (status == google.maps.StreetViewStatus.OK) {
                        ...
                        if (panorama_request.radius > 50000) {
                            set_random_coords(); // If radius is more than 50km, choose new location.
                        } else {
                            panorama_request.radius += 5000; // If not, just increase radius and repeat search.
                            find_panorama();
                        }
                    }
                });
